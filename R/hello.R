...
genomplot <- function(x,y,z) {
  ...
  postart<-tissue%>%left_join(m,by='gene')%>%mutate(P=-log10(pvalue),
                                                    ...
                                                    gene_type=paste(gene_name,tissue, sep = "_"))%>%filter(CHR==8)%>%
